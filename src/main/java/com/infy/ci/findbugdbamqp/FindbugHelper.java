...
public class FindbugHelper {
	 static Logger log = Logger.getLogger(FindbugHelper.class.getName());
	
	static private FindbugHelper cihelper;
	private final static Object cihelperLock = new Object();
	private FindbugHelper(){
		
		
	}
	
	public static FindbugHelper getInstance(){
		synchronized (cihelperLock) {
			if(null == cihelper){
				cihelper = new FindbugHelper();
			}
		}
		return cihelper;
	}
